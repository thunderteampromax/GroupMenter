...
async def _(event):
    ...
    lool = 0
    KkK = await event.reply("`🔎 Finding the Book`")
    lin = "https://b-ok.cc/s/"
    text = input_str
    link = lin + text
    ...
    if nbx == "0":
        ...
        f.write("By @TeamThunderSupport.")
        f.close()
        caption = "Support Chat \n Join Support @thundergotechnologysupport"

        await tbot.send_file(
            event.chat_id,
            "book.txt",
            caption=f"**BOOKS GATHERED SUCCESSFULLY!**\n\nBY  @thundergotechnologysupport.\n\n JOIN THE UPDATES CHANNEL 👉 @thunderprojectsupdates.",
        )
        os.remove("book.txt")
        await KkK.delete()
